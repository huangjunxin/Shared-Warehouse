...
- **Reservation Order Detail**: Located at `client/src/pages/ReservationOrderDetail.tsx`, uses unified sub-page Header with ← back button.
  ...
  - **Reservation items**: Displayed in two-column grid (`ReservationGrid`, `grid-template-columns: repeat(2, 1fr)`). Each card shows status tag at top-left, item name below it (with `word-break: break-all`), location, time range (font-size: 12px, color: #333, font-weight: 500, separated by border-top). Cancel button is a red circular X (`CancelBtn`) positioned at top-right of card (only for owner, non-canceled reservations). Location display: items in user's personal box show "在我手中" (green text) for current user, "XXX手中" for other users; regular boxes show "room_name / box_name". Backend returns `is_user_box`, `holder_nickname`, `holder_user_id` fields. No calendar emoji before time.
